...
* extracting the data of these strains from the analysis dataset
...
The **1-100 well** in each raw file is the growth data of the strains in a plate at **basal condition**. From **101-200 well** have the data of those strains at acetic acid concentration as indicated in the raw file name. The the wells can be linked to the strain names (gRNA names) using the **Plate layout** file as mentioned above in [VALIDATION DATA ANALYSIS]. 
...
* Extracting the data of control strains

The dataset (**Val_data**) until now have the data for all well  

#Extracting the data of All dummy control strains
...
#Now the dCTRL strains were in both plates. Therefore, we create a new data frame and extract 
#only the required columns to perform the analysis
#
...
####################################################################################
#Checking the correlation between the scan-o-matic LPI and Bioscreen LPI response ##
####################################################################################
####################################################################################

#Removing the dCTRL data.Remember that each dCTRL are represented twice in the dataset 
#as the dCTRL strains are present in both plates (That makes 6 independent replicates each). 
#We will add for all dCTRL the recalculated Mean, sd, and N after taking all the 6 replicates in to account

Val_data_dCTRl_removed <- Val_data[-dCTRL_rows, ]

#Removing also the Blank rows from the data
Val_data_dCTRl_removed <- Val_data_dCTRl_removed[-which(Val_data_dCTRl_removed$gRNA_name=="BLANK"), ]

#Trimming all the non-essential column for correlation analysis
Val_data_dCTRl_removed <- Val_data_dCTRl_removed[, c(2:4, 131:148)]

Validation_LPI_all <- rbind(Val_data_dCTRl_removed, Val_data_dCTRL_F)

write.csv(Validation_LPI_all, "Validation_LPI_all.csv")

rownames(Validation_LPI_all) <- Validation_LPI_all$gRNA_name

###############################################################
#Extracting the some additional data from the scan-o-matic data
###############################################################

#Previously we made a dataset that included scan-o-matic analysis data of all strains selectd for validation
#ultimately we decided to remove two strains out of this list as they were growing poorly in the basal medium. 

a <- setdiff(validation_strains_data$gRNA_name, Validation_LPI_all$gRNA_name)
validation_strains_data <- validation_strains_data[!(row.names(validation_strains_data) %in% a), ]

#Now we will add a new column with a new identifier that indicates the 50 most acetic acid tolerant
#and acetic acid sensitive strains in this list. We already had a dataframe with the list of this strains
#Once again we removed the two strains those werefinally eliminated from the validation experiment
#Therefore, we had 48 sensitive strains and 50 tolerant strains included in the validation experiment, 

bot_top_50 <- bot_top_50[!(bot_top_50$gRNA_name %in% a), ]
...
#Row Binding the two datasets

validation_strains_data <- rbind(validation_strains_data, dCTRL_data_scan_o_matic)

#setting the row order similar to that of Validation_LPI_all dataset

validation_strains_data <- validation_strains_data[rownames(Validation_LPI_all), ]

#Now making a new data frame by extracting only the required column from both data.frame
#i.e. the data.frame that has the scan-o-matic analysis for the validation strains (validation_strains_data) 
#and the data.frame that has the data of the bioscreen liquid growth experiment (Validation_LPI_all)
